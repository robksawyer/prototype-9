...
describe('components', () => {
  describe('NurbsLine', () => {
    it('should mount with props', function () {
      const wrap = mount(<NurbsLine>Hello World</NurbsLine>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<NurbsLine>Hello World</NurbsLine>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('nurbs_line')
