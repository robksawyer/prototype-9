...
describe('components', () => {
  describe('NurbsObject', () => {
    it('should mount with props', function () {
      const wrap = mount(<NurbsObject>Hello World</NurbsObject>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<NurbsObject>Hello World</NurbsObject>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('nurbs_object')
